...
//     key: fs.readFileSync('/etc/letsencrypt/live/fpstech.siswork.com/privkey.pem'),
//     cert: fs.readFileSync('/etc/letsencrypt/live/fpstech.siswork.com/fullchain.pem'),
...
app.get("/product-list", async(req,res)=> {
  await client.connect();
  const db = client.db('fps_technology');
  const collection =  db.collection('product_client');
  ...
  const paginatedItems = plist.slice(startIndex, endIndex);
  const totalPages = Math.ceil(plist.length / limit);
...
app.get("/pdetails", async (req, res) => {
  ...
  //   .limit(50)
  //   .toArray();
  let results = await collection.find({"product_name":{$regex : query , $options : "i"}}).toArray();
